...
Collection.prototype.stream = function find(criteria, stream) {
  ...
  // Run Normal Query on collection (MongoDB 6.20+ compatible with backpressure)
  ...
  (async function processStream() {
    try {
      for await (const item of cursor) {
        ...
        // Maintain backpressure control like original .stream()
        await new Promise(function(resolve, reject) {
          stream.write(obj, function(err) {
            if (err) reject(err);
            else resolve();
          });
        });
...
Collection.prototype.count = async function count(criteria, cb) {
  ...
  try {
    const count = await this.connection.db
      .collection(this.identity)
      .countDocuments(query.criteria.where);
    cb(null, count);
  } catch (err) {
    if (err) return cb(err);
  }
