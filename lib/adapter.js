...
module.exports = (function () {
  ...
  var adapter = {
    ...
    defaults: {
      ...
      // Write Concern Options (MongoDB 6.20+)
      w: 1,
      wTimeoutMS: 0,
      journal: false,
      readPreference: null,

      // Retry Options (MongoDB 6.20+)
      retryWrites: true,
      retryReads: true,

      // SSL/TLS Options (MongoDB 6.20+)
      ssl: false,
      tls: false,
      tlsCAFile: null,
      ...
      // Connection Pool Options (MongoDB 6.20+)
      maxPoolSize: 5,
      minPoolSize: 1,
      maxIdleTimeMS: 0,
      ...
      // Connection Timeout Options (MongoDB 6.20+)
      ...
      // Legacy compatibility (kept for backward compatibility, mapped to new options)
      poolSize: 5, // maps to maxPoolSize
      wtimeout: 0, // maps to wTimeoutMS
    ...
    registerConnection: function (connection, collections, cb) {
      ...
      new Connection(connection, function (_err, db) {
        if (_err) {
          return cb(
            (function _createError() {
              var msg = util.format(
                "Failed to connect to MongoDB.  Are you sure your configured Mongo instance is running?\n Error details:\n%s",
                util.inspect(_err, false, null)
              );
        ...
        Object.keys(collections).forEach(function (key) {
          connections[connection.identity].collections[key] = new Collection(
            collections[key],
            db
          );
        });
    ...
    teardown: async function (conn, cb) {
      if (typeof conn == "function") {
        cb = conn;
        conn = null;
      }
      if (conn === null) {
        var clients = Object.values(connections).map(
          (connectionValue) => connectionValue.connection.client
        );
        if (!clients.length) {
          return cb();
        }
    ...
    define: function (connectionName, collectionName, definition, cb) {
      ...
      connectionObject.connection.createCollection(
        collectionName,
        collection,
        cb
      );
    ...
    drop: async function (connectionName, collectionName, relations, cb) {
      ...
      try {
        await connectionObject.connection.db.dropCollection(collectionName);
        cb();
      } catch (e) {
        // Don't error if droping a collection which doesn't exist
        if (e && e.errmsg === "ns not found") {
          return cb();
        }
        cb(e);
      }
      ...
      _runJoins(
        {
          ...
          $getPK: function (collectionIdentity) {
            ...
            if (!connectionObject) {
              throw new Error(
                "Consistency violation in sails-mongo: Unrecognized datastore (i.e. connection): `" +
                  connectionName +
                  "`."
              );
            }
            ...
            if (!collection) {
              throw new Error(
                "Consistency violation in sails-mongo: Unrecognized collection: `" +
                  collectionIdentity +
                  "` in datastore (i.e. connection): `" +
                  connectionName +
                  "`."
              );
            }
