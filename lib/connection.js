...
Connection.prototype._ensureIndexes = async function _ensureIndexes(
  ...
  const a = indexes.map((item) => {
    console.log("_ensureIndexes - item.index", item.index);
    return collection.createIndex(item.index, item.options);
    // console.log("_ensureIndexes - item.index", item.index, "done");
  });
  ...
  try {
    ...
    console.log("_ensureIndexes-catch");
    if (err.code !== 86 && err.code !== 85) {
      console.log("_ensureIndexes-catch-if");
      cb && cb(err);
    }
  }
  console.log("_ensureIndexes- done");
};
