...
Query.prototype.parseClause = function parseClause(original) {
  ...
  return _.reduce(
    original,
    function parseClausePair(obj, val, key) {
      "use strict";
      ...
      // handle Like Operators for WQL (Waterline Query Language)
        ...
        _.extend(
          ...
          _.reduce(
            val,
            function parseLikeClauses(likes, expression, field) {
  ...
  if (_.isPlainObject(expression) && !_.isDate(expression)) {
    return _.reduce(
      ...
      function (obj, val, modifier) {
        ...
        if (
          modifier === "<=" ||
          modifier === "lessThanOrEqual" ||
          modifier.toLowerCase() === "lte"
        ) {
        ...
        if (
          modifier === ">=" ||
          modifier === "greaterThanOrEqual" ||
          modifier.toLowerCase() === "gte"
        ) {
  ...
  if (hop(self.schema, field) && self.schema[field].type === "objectid") {
    ...
    if (_.isArray(val)) {
      return _.map(val, function (item) {
        return _.isString(item) && utils.matchMongoId(item)
          ? new ObjectId(item)
          : item;
      });
    }
